...
def train(agent_h, agent_c, env, LTL_model, gamma, zeta, nn_num):
    ...
    for i_episode in range(num_episodes):
        ...
        if score > max_score:
            ...
            agent_c.save('dir_chk/HCPS-LTL/grid_QL_zeta', 'GridWorldMax')
            max_score = score
        ...
        if i_episode % print_every == 0 and i_episode > 0:
            print(
                'Episode: {:5} Score: {:5}  Avg.Score: {:.8f}, Success_Rate: {:.2f}, eps-greedy: {:5.2f}, Time: {:02}:{:02}:{:02}'. \
...
def run_episode(agent_h, agent_c, env, LTL_model, gamma, zeta, episode):
    ...
    while iteration < 100: #and \
        ...
        # env.render()
        next_q, reward = excution_LTL(label_action, str(temp_q), LTL_model, zeta)
...
def excution_LTL(action, temp_q, LTL_model, zeta):
    # next_q = temp_q
    # lable = 'd'
    reward = 0
    # Gamma = gamma
...
def main(env, agent_h, agent_c, LTL_model, gamma, zeta, nn_num):
    ...
    agent_c.save('dir_chk/HCPS-LTL/grid_QL_zeta', 'GridWorld')
...
if __name__ == '__main__':
    ...
    agent_h = Reinforce(alpha=0.0001, state_dim=space_dim, action_dim=action_dim,
                      fc1_dim=16, fc2_dim=16, ckpt_dir='dir_chk/Reinforce_HPS/GridHCPS-2-zeta/', gamma=0.99)
