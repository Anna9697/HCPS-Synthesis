...
#                     fc1_dim=hidden_dim, fc2_dim=hidden_dim, ckpt_dir='dir_chk/Reinforce_HPS/HCPS-3/', gamma=0.99)
agent_c = Agent(state_dim + 1, 2, seed=0)
print('input_dim: ', state_dim, ', output_dim: ', action_dim, ', hidden_dim: ', hidden_dim)

# agent_h.load_models('dir_chk/Reinforce_HPS/GridHCPS-1/', 1)
# agent_c.load_nn('dir_chk/HCPS-LTL/grid1/', 'GridWorld')
...
def play(env, n_episodes):
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        for i in range(100):
            ...
            next_q, reward, Gamma = excution_LTL(env, s2, str(temp_q), LTL_model, 0.9999, 0.99)  # 自定义LTL自动机状态转换函数
