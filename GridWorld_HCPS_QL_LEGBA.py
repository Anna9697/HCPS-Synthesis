...
def train(agent_h, agent_c, env, LTL_model, gamma, gammaB, nn_num):
    ...
    for i_episode in range(num_episodes):
        ...
        if score > max_score:
            ...
            agent_c.save('dir_chk/HCPS-LTL/grid_QL_LDGBA', 'GridWorldMax')
            max_score = score
...
def excution_LTL(action, temp_q, LTL_model, gamma, gammaB):
    ...
    if LTL_model.get_reward_label(next_q):
        reward += 1 - gammaB
        Gamma = gammaB
    # print(next_q)
...
def main(env, agent_h, agent_c, LTL_model, gamma, gammaB, nn_num):
   ...
   agent_c.save('dir_chk/HCPS-LTL/grid_QL_LDGBA', 'GridWorld')

if __name__ == '__main__':
    LTLpath = "resources/LDBA/LTL_gridworld1.json"
    LTL_model = LDGBAUtil.LDBA_Util(LTLpath)
    LDGBA.make_model(LTL_model, "resources/LDGBA/", "LTL_gridworld1")
    ...
    agent_h = Reinforce(alpha=0.0001, state_dim=space_dim, action_dim=action_dim,
                      fc1_dim=16, fc2_dim=16, ckpt_dir='dir_chk/Reinforce_HPS/GridHCPS-2-LDGBA/', gamma=0.99)
